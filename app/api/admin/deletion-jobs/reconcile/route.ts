...
export async function POST(request: Request) { try { const auth = await requireServerPermission(request, "assessment:delete"); const result = await reconcileDueDeletionJobs({ jobs: getDeletionJobRepository(), lifecycle: getAssessmentLifecycleRepository(), storage: getArtifactStorage(), scope: scopeFromTenant(auth.tenant), actorUserId: auth.user.id, limit: 20 }); return NextResponse.json(result, { headers: { "Cache-Control": "no-store" } }); } catch (error) { if (error instanceof AuthenticationRequiredError) return NextResponse.json({ error: error.message }, { status: 401, headers: { "Cache-Control": "no-store" } }); if (error instanceof AuthorizationDeniedError) return NextResponse.json({ error: error.message }, { status: 403, headers: { "Cache-Control": "no-store" } }); throw error; } }
