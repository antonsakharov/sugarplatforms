...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const readiness = {
    status: readyForAnalysis ? "ready" : "review_required", readyForAnalysis, totalPages: measurablePages, unmeasurableFiles, warningCount,
    message: readyForAnalysis ? "Artifact set passed upload readiness checks and is ready for parsing." : "Replace or remove flagged artifacts before analysis. Page counts and content-risk scanning are best-effort and do not guarantee sensitive-data detection."
  } as const;
  ...
  if (!readyForAnalysis) {
    parsing.processingArtifacts = files.map((file) => ({ artifactName: file.name, status: "withheld", message: "Parsing withheld until upload readiness issues are resolved." }));
  } else {
    for (const file of files) {
      try {
        ...
        parsing.errors.push({ artifactName: file.name, message });
        parsing.processingArtifacts.push({ artifactName: file.name, status: "failed", message });
      }
    }
