...
type AiCandidates = { stats: { candidateCount: number } };
...
export function AssessmentWorkspace({ id }: { id: string }) {
  ...
  const [aiCandidates, setAiCandidates] = useState<AiCandidates | null>(null);
  ...
  useEffect(() => {
    ...
    const aiCandidatesRaw = localStorage.getItem(`sugar:ai-candidates:${id}`);
  ...
  return <>
    ...
    <div className="workspace-grid">
      ...
      <article className={`card ${review?.approved && !findingsComplete ? "current-step" : ""}`}><span className="card-number">04</span><h3>Diagnose & review</h3><p>{findingsComplete ? `${acceptedCount} accepted finding${acceptedCount === 1 ? "" : "s"} ready for downstream outputs.` : diagnostics ? `${diagnostics.stats.findingCount} evidence-backed deterministic finding${diagnostics.stats.findingCount === 1 ? "" : "s"} · ${aiCandidateCount} isolated AI-assisted candidate${aiCandidateCount === 1 ? "" : "s"}.` : review?.approved ? "Extraction approved; deterministic diagnostics are ready." : "Evidence-backed findings and human decisions."}</p></article>
