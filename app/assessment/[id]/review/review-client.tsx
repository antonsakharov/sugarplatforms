    ...
    <div className="panel">
      ...
      <div className="artifact-list">{review.objects.map((item) => {
        ...
        return <article className={`inspection-row status-${item.status === "rejected" ? "blocked" : item.status === "pending" ? "review_required" : "validated"}`} key={item.id}>
          ...
          <label><span>Reviewed name</span><input value={item.displayName} disabled={saving || review.approved || item.status === "merged"} onChange={(event) => mutate(() => renameReviewedObject(review, item.id, event.target.value))} /></label>
