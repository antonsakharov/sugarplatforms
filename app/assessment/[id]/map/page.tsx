...
export default function EntityIdMapPage() {
  ...
  return <>
    ...
    <p className="lede">This projection uses only confirmed extraction objects and accepted findings from a completed, non-stale review. Creator, consumer, authority, and integration relationships appear only when the uploaded architecture evidence states them explicitly; scope-derived relationships remain labeled separately.</p>
    <div className="metrics diagnostic-metrics"><article><strong>{graph.stats.nodeCount}</strong><span>nodes</span></article><article><strong>{graph.stats.edgeCount}</strong><span>relationships</span></article><article><strong>{graph.stats.directRelationshipCount}</strong><span>direct entity roles</span></article><article><strong>{graph.stats.acceptedFindingCount}</strong><span>accepted findings</span></article><article><strong>{graph.stats.evidenceReferenceCount}</strong><span>evidence links</span></article></div>
