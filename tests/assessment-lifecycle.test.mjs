...
function seedAssessment(databasePath) {
  ...
  assessments.create(scope, { id: assessmentId, companyName: "Example", assessmentTitle: "Delete me", industry: "Technology", focusArea: "entity-identifier-fragmentation", primaryEntity: "Customer", knownSystems: "CRM", businessConcern: "Fragmentation", reportAudience: "CTO", limitsAcknowledged: true, status: "draft", createdAt });
...
test("admin deletion removes private objects and all assessment-scoped state but retains audit receipts", async () => {
  await withFixture(async ({ root, databasePath }) => {
    ...
    const stored = await storage.put(scope, assessmentId, { originalName: "architecture.md", mediaType: "text/markdown", bytes, checksumSha256 });
    ...
    db.prepare(`INSERT INTO artifact_metadata (organization_id, workspace_id, assessment_id, storage_artifact_id, parser_artifact_id, original_name, media_type, size_bytes, checksum_sha256, parser, warnings_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run("org_a", "ws_a", assessmentId, stored.id, "parser-a", "architecture.md", "text/markdown", bytes.length, checksumSha256, "text", "[]", createdAt);
    db.prepare(`INSERT INTO extraction_snapshots (organization_id, workspace_id, assessment_id, schema_version, provider, prompt_version, status, payload_json, persisted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run("org_a", "ws_a", assessmentId, "1.0", "local", "v1", "complete", "{}", createdAt);
    db.prepare(`INSERT INTO extraction_reviews (organization_id, workspace_id, assessment_id, extraction_fingerprint, review_json, updated_at) VALUES (?, ?, ?, ?, ?, ?)`)
      .run("org_a", "ws_a", assessmentId, "a".repeat(64), "{}", createdAt);
    db.prepare(`INSERT INTO finding_reviews (organization_id, workspace_id, assessment_id, diagnostic_fingerprint, diagnostics_json, review_json, accepted_findings_json, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run("org_a", "ws_a", assessmentId, "b".repeat(64), "{}", "{}", "[]", createdAt);
    db.prepare(`INSERT INTO report_snapshots (organization_id, workspace_id, assessment_id, report_id, version, diagnostic_generated_at, snapshot_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run("org_a", "ws_a", assessmentId, "report-a", 1, createdAt, "{}", createdAt);
    db.close();

    const lifecycle = new SqliteAssessmentLifecycleRepository(databasePath);
    const receipt = await deleteAssessmentWithAudit({ repository: lifecycle, storage, scope, assessmentId, actorUserId: "admin_user" });
    ...
    const events = lifecycle.listAudit(scope, assessmentId);
    ...
    assert.equal(lifecycle.listAudit(otherScope, assessmentId).length, 0);
  });
});
...
test("cross-tenant deletion fails closed", async () => {
  await withFixture(async ({ root, databasePath }) => {
    ...
    const lifecycle = new SqliteAssessmentLifecycleRepository(databasePath);
    const storage = new LocalPrivateArtifactStorage(join(root, "private"));
    await assert.rejects(() => deleteAssessmentWithAudit({ repository: lifecycle, storage, scope: otherScope, assessmentId, actorUserId: "admin_user" }), AssessmentDeletionNotFoundError);
    assert.equal(lifecycle.listAudit(scope, assessmentId).length, 0);
  });
});
...
test("storage failure records a failure receipt and leaves database assessment state intact", async () => {
  await withFixture(async ({ databasePath }) => {
    ...
    const db = new DatabaseSync(databasePath);
    db.prepare(`INSERT INTO artifact_metadata (organization_id, workspace_id, assessment_id, storage_artifact_id, parser_artifact_id, original_name, media_type, size_bytes, checksum_sha256, parser, warnings_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run("org_a", "ws_a", assessmentId, "artifact-a", "parser-a", "a.md", "text/markdown", 4, "0".repeat(64), "text", "[]", createdAt);
    db.close();
    const lifecycle = new SqliteAssessmentLifecycleRepository(databasePath);
    ...
    await assert.rejects(() => deleteAssessmentWithAudit({ repository: lifecycle, storage, scope, assessmentId, actorUserId: "admin_user" }), /object store unavailable/);
    assert.notEqual(assessments.findById(scope, assessmentId), null);
    const events = lifecycle.listAudit(scope, assessmentId);
