...
export const authenticatedContextSchema = z.object({
  ...
  if (value.membership.userId !== value.user.id) {
    context.addIssue({ code: "custom", path: ["membership", "userId"], message: "Membership must belong to the authenticated user." });
  }
  if (value.membership.organizationId !== value.tenant.organization.id || value.membership.workspaceId !== value.tenant.workspace.id) {
    context.addIssue({ code: "custom", path: ["membership"], message: "Membership must match the active organization/workspace." });
  }
});
...
export type Permission = "assessment:read" | "assessment:create" | "tenant:read";

const ROLE_PERMISSIONS: Record<Role, ReadonlySet<Permission>> = {
  viewer: new Set(["assessment:read", "tenant:read"]),
  editor: new Set(["assessment:read", "assessment:create", "tenant:read"]),
  admin: new Set(["assessment:read", "assessment:create", "tenant:read"])
};

export class AuthenticationRequiredError extends Error {
  constructor() {
    super("Authenticated access is required.");
...
export class AuthorizationDeniedError extends Error {
  constructor(permission: Permission) {
    super(`The current membership does not allow ${permission}.`);
