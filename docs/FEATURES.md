...
Status: implemented through formal PDF export with server-reviewed source hydration — the report preview now reloads the authenticated current assessment, validated artifact metadata, extraction/review boundary, diagnostics, and completed accepted-finding state from the server, then deterministically regenerates maturity, recommendations, and the 90-day plan. Stale/incomplete reviewed state fails closed instead of falling back to browser findings. Explicit report snapshots/history remain browser-local with monotonic versions, JSON export, print styling, and formal PDF generation from immutable snapshots. Durable tenant-scoped report history, authorization, private report storage, signing, and audit/deletion remain open.
